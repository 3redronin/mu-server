...
class LegacyMuVariantListBuilder extends Variant.VariantListBuilder {
    ...
    static Variant selectVariant(List<Variant> available, List<Locale.LanguageRange> preferredLanguages, List<MediaType> acceptableMediaTypes, List<ParameterizedHeaderWithValue> acceptableEncodings) {
        ...
        for (Variant candidate : available) {
            ...
            boolean mtOkay = cmt == null || LegacyMediaTypeHeaderDelegate.atLeastOneCompatible(singletonList(cmt), acceptableMediaTypes, "charset");
    ...
    private static MediaType bestMediaType(List<MediaType> acceptableMediaTypes, Collection<MediaType> candidates) {
        ...
        Map<LegacyCombinedMediaType, MediaType> combinedToServerType = new HashMap<>();
        List<LegacyCombinedMediaType> m = new ArrayList<>();
        for (MediaType clientAccepts : acceptableMediaTypes) {
            for (MediaType serverProduces : candidates) {
                if (clientAccepts.isCompatible(serverProduces)) {
                    LegacyCombinedMediaType combined = LegacyCombinedMediaType.s(clientAccepts, serverProduces);
    ...
    private static double bestEncodingScore(List<ParameterizedHeaderWithValue> acceptableEncodings, Variant variant) {
        return acceptableEncodings.stream().filter(enc -> variant.getEncoding().equals(enc.value())).min((o1, o2) -> Double.compare(qScore(o2), qScore(o1))).map(LegacyMuVariantListBuilder::qScore).orElse(1.0);
    }
