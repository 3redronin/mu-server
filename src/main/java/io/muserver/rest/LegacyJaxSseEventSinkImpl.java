...
class LegacyJaxSseEventSinkImpl implements SseEventSink {
    private static final Logger log = LoggerFactory.getLogger(LegacyJaxSseEventSinkImpl.class);

    private final AsyncSsePublisher ssePublisher;
    private final MuResponse response;
    private final EntityProviders entityProviders;

    public LegacyJaxSseEventSinkImpl(AsyncSsePublisher ssePublisher, MuResponse response, EntityProviders entityProviders) {
        ...
        try {
            ...
            if (event.getData() != null) {
                MessageBodyWriter messageBodyWriter = entityProviders.selectWriter(event.getType(), event.getGenericType(),
                    JaxRSResponse.Builder.EMPTY_ANNOTATIONS, MediaType.valueOf(event.getMediaType().toString()));
                try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
                    messageBodyWriter.writeTo(event.getData(), event.getType(), event.getGenericType(), JaxRSResponse.Builder.EMPTY_ANNOTATIONS,
                        MediaType.valueOf(event.getMediaType().toString()), muHeadersToJaxObj(response.headers()), out);
