...
class LegacyMuPathSegment implements PathSegment {
    private final String path;
    private final MultivaluedMap<String, String> params;

    LegacyMuPathSegment(String path, MultivaluedMap<String, String> params) {
        this.params = params;
        this.path = path;
    }
    ...
    public boolean equals(Object o) {
        ...
        LegacyMuPathSegment that = (LegacyMuPathSegment) o;
    ...
    public List<LegacyMuPathSegment> resolve(String name, String value, boolean encodeSlashInPath) {
        ...
        if (encodeSlashInPath) {
            return Collections.singletonList(new LegacyMuPathSegment(newPath, newParams));
        }
        String[] newPaths = newPath.split("/");
        List<LegacyMuPathSegment> list = new ArrayList<>();
        for (int i = 0; i < newPaths.length; i++) {
            String s = newPaths[i];
            MultivaluedMap<String, String> p = i == 0 ? newParams : LegacyReadOnlyMultivaluedMap.empty();
            LegacyMuPathSegment muPathSegment = new LegacyMuPathSegment(s, p);
            list.add(muPathSegment);
        }
        return list;
    }

    LegacyMuPathSegment toEncoded() {
        ...
        return new LegacyMuPathSegment(urlEncode(this.path), copy);
    }
}
