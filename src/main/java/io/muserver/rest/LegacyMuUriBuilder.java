...
class LegacyMuUriBuilder extends UriBuilder {
    static {
    ...
    private String host;
    private int port = -1;
    private List<LegacyMuPathSegment> pathSegments = new ArrayList<>();
    ...
    private LegacyMuUriBuilder(String scheme, String userInfo, String host, int port, List<LegacyMuPathSegment> pathSegments,
    ...
    public UriBuilder clone() {
        return new LegacyMuUriBuilder(scheme, userInfo, host, port, pathSegments, hasPrecedingSlash, hasTrailingSlash, query, fragment);
    }
    ...
    public UriBuilder schemeSpecificPart(String ssp) {
        LegacyMuUriBuilder builder = (LegacyMuUriBuilder) fromUri(URI.create(scheme + "://" + ssp));
    ...
    public UriBuilder path(String path) {
        ...
        setSlashes(path);
        this.pathSegments.addAll(LegacyMuUriInfo.pathStringToSegments(decode(path), false).collect(toList()));
        return this;
    }
    ...
    public UriBuilder segment(String... segments) {
        ...
        for (String segment : segments) {
            ...
            this.pathSegments.addAll(LegacyMuUriInfo.pathStringToSegments(decode(segment), true).collect(toList()));
        }
    ...
    private LegacyMuPathSegment getOrCreateCurrentSegment() {
        LegacyMuPathSegment ps;
        if (pathSegments.isEmpty()) {
            ps = new LegacyMuPathSegment("", new MultivaluedHashMap<>());
            pathSegments.add(ps);
        } else {
    ...
    public URI buildFromMap(Map<String, ?> values, boolean encodeSlashInPath) throws IllegalArgumentException, UriBuilderException {
        LegacyMuUriBuilder resolved = cloneAndResolve(values, encodeSlashInPath, false);
    ...
    public URI buildFromEncodedMap(Map<String, ?> values) throws IllegalArgumentException, UriBuilderException {
        LegacyMuUriBuilder resolved = cloneAndResolve(values, true, true);
        return URI.create(resolved.buildIt(Mutils::urlEncode));
    ...
    private LegacyMuUriBuilder cloneAndResolve(Map<String, ?> values, boolean encodeSlashInPath, boolean valuesAlreadyEncoded) {
        if (values.isEmpty()) {
            return this;
        }
        LegacyMuUriBuilder copy = (LegacyMuUriBuilder) clone();
    ...
    private Map<String, ?> valuesToMap(Object[] values) {
        ...
        for (LegacyMuPathSegment pathSegment : pathSegments) {
