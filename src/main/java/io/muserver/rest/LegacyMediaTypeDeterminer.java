...
class LegacyMediaTypeDeterminer {
    public static MediaType determine(LegacyObjWithType responseObject, List<MediaType> classProduces, List<MediaType> methodProduces, List<LegacyProviderWrapper<MessageBodyWriter<?>>> messageBodyWriters, List<MediaType> clientAccepts, Annotation[] annotations) {
        ...
        if (!methodProduces.isEmpty()) {
            ...
            p = messageBodyWriters.stream()
                .filter(writer -> writer.provider.isWriteable(responseObject.type, responseObject.genericType, annotations, MediaType.WILDCARD_TYPE))
        ...
        for (MediaType mediaTypeA : a) {
            ...
            for (MediaType mediaTypeP : p) {
                ...
                if (mediaTypeA.isCompatible(mediaTypeP)) {
                    m.add(LegacyCombinedMediaType.s(mediaTypeA, mediaTypeP));
                }
            }
        }
        ...
        for (LegacyCombinedMediaType mediaType : m) {
