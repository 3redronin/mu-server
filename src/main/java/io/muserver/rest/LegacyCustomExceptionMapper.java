...
class LegacyCustomExceptionMapper {
    private static final Logger log = LoggerFactory.getLogger(LegacyCustomExceptionMapper.class);

    private final Map<Class<? extends Throwable>, ExceptionMapper<? extends Throwable>> mappers;

    LegacyCustomExceptionMapper(Map<Class<? extends Throwable>, ExceptionMapper<? extends Throwable>> mappers) {
    ...
    Response toResponse(Throwable ex) {
        ...
        if (exClass.equals(LegacyJaxRSRequest.FilterAbortedException.class)) {
            return ((LegacyJaxRSRequest.FilterAbortedException)ex).getResponse();
        }
