...
`Method.QUERY` and `io.muserver.rest.QUERY` support safe, idempotent queries with request content
([RFC 10008](https://www.rfc-editor.org/rfc/rfc10008.html)). Native routes use the normal body APIs:

```java
MuServer server = MuServerBuilder.httpServer()
    .addHandler(Method.QUERY, "/search", (request, response, pathParams) -> {
        String query = request.readBodyAsString();
        response.contentType("text/plain");
        response.write("Result for " + query);
    }).start();
```

A JAX-RS resource declares its input and result formats in the usual way:
...
Application annotations carrying `@HttpMethod("QUERY")` also work, including inherited resource methods.
QUERY requires a valid, concrete `Content-Type`, even when its body is empty. Missing, blank or malformed
values are rejected with 400 before handlers or `100 Continue`; the server does not infer a type.
Providers and resources interpret the content. Normal negotiation returns 415 for unsupported input and
406 for unacceptable output. URI query parameters remain separate from the body.

Automatic OPTIONS responses advertise QUERY in `Allow` and list effective `@Consumes` formats in
`Accept-Query`. Framework-generated QUERY 415 responses also advertise these formats. The header is a
Structured Fields list, for example `"application/json", "text/plain";charset="UTF-8"`. Formats from matching
overloads and subresources are combined, sorted and deduplicated, including inherited declarations.
An undeclared consumes type is `"*/*"`; ordinary `type/*` ranges are preserved. If any declaration cannot
be faithfully represented (for example `application/*+json`), the automatic header is omitted.
Application-supplied headers and explicit OPTIONS behavior take precedence. CORS preflights advertise
QUERY when configured; browsers can read `Accept-Query` only if the CORS exposed-header configuration allows it.
QUERY-only resources do not gain GET or HEAD support.

Conditional QUERY results can use `Request.evaluatePreconditions`: matching `If-None-Match` (including
weak tags) and unchanged `If-Modified-Since` yield 304 with no response body. Failed `If-Match` and
`If-Unmodified-Since` yield 412. Supply validators for the selected query result. QUERY is exempt from
CSRF safe-method checks. The HTTPS redirector continues to reject HTTP QUERY with 400 and directs clients
to HTTPS. Query execution, caching, stored results and application redirects remain resource-controlled.
Generated OpenAPI 3.2.1 documents use `paths.<path>.query`, including request bodies and response formats;
the HTML documentation provides QUERY curl examples.
