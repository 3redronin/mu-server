...
import static ronin.muserver.rest.MediaTypeHeaderDelegate.canProduceForClient;
...
public class MediaTypeHeaderDelegateTest {
    ...
    public void ifClientAcceptsWildcardThenAnythingGoes() {
        assertThat(canProduceForClient(types("image/jpeg"), types("*/*")), is(true));
        assertThat(canProduceForClient(types("image/*", "text/plain"), types("*/*")), is(true));
    }

    @Test
    public void ifClientAcceptsSubTypeWildcardsThenThoseCanBeServiced() {
        assertThat(canProduceForClient(types("image/jpeg"), types("image/*")), is(true));
        assertThat(canProduceForClient(types("image/*", "text/plain"), types("image/*")), is(true));
        assertThat(canProduceForClient(types("text/*"), types("image/*")), is(false));
    }

    @Test
    public void wildcardsCannotProvideForSpecificTypes() {
        assertThat(canProduceForClient(types("*/*"), types("image/svg+xml", "image/jpeg")), is(true));
        assertThat(canProduceForClient(types("image/*"), types("image/svg+xml", "image/jpeg")), is(true));
        assertThat(canProduceForClient(types("image/*"), types("*/*")), is(true));
        assertThat(canProduceForClient(types("image/*"), types("text/*")), is(false));
    }
