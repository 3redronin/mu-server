...
public class Http1Client implements AutoCloseable {
    ...
    static {
        TrustManager[] trustAllCertificates = new TrustManager[]{veryTrustingTrustManager()};
        try {
    ...
    public Http1Client writeRequestLine(Method method, String uri) {
        return writeRequestLine(method, server.resolve(uri), "HTTP/1.1", true);
    }
    public Http1Client writeRequestLine(Method method, URI uri, String httpVersion, boolean writeHost) {
        writeAscii(method.name() + " " + uri.getRawPath() + " " + httpVersion + "\r\n");
        if (writeHost) {
