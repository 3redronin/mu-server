...
The executable [field checklist](../src/test/resources/openapi/field-coverage.tsv) is exercised by `OpenApi31FieldCoverageTest`. The current offline resources in `src/test/resources/openapi` include the official OpenAPI 3.2 schema and schema-base dated 2026-08-30, the OAS dialect dated 2026-02-26, and Draft 2020-12 metaschemas. Their canonical source URLs and exact SHA-256 digests are in that directory's `manifest.json`. Test-only NetworkNT 1.5.9 validates complete documents and representative generated-schema payloads; unknown resource fetches fail instead of reaching the network.
